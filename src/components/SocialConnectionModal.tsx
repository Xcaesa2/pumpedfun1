...
const SocialConnectionModal: React.FC<SocialConnectionModalProps> = ({ user, platform, onClose }) => {
  ...
  const { login, user: kindeUser, isAuthenticated } = useKindeAuth()
  ...
  }, [user?.id, connectionsLoading, initialLoadComplete, loadConnections])
  ...
  }, [platform, initialLoadComplete, telegramWidgetLoaded, loadTelegramWidget])
  ...
  // Auto-link X (Twitter) account after Kinde login if not already connected
  useEffect(() => {
    if (
      platform === 'x' &&
      isAuthenticated &&
      ...
      // Safely check for identities property
      const identities = (kindeUser as any)?.identities;
      if (Array.isArray(identities)) {
        ...
        if (xIdentity) {
          addConnection({
            user_id: user.id,
            platform: 'x',
            platform_user_id: xIdentity.user_id || xIdentity.id || '',
            platform_username: xIdentity.username || xIdentity.screen_name || '',
            is_active: true,
          });
        }
      }
    }
  }, [platform, isAuthenticated, kindeUser, connected, addConnection, user.id]);
      ...
      >
        ...
        {alreadyLinked && (
          <div className="mb-4 p-3 rounded-lg bg-yellow-500/10 border border-yellow-500/30 flex items-center space-x-2">
            <AlertCircle className="w-4 h-4 text-yellow-400" />
            <span className="text-sm text-yellow-400">
              You have already linked your X (Twitter) account.
            </span>
            <button
              onClick={() => setAlreadyLinked(false)}
              className="ml-auto text-xs text-yellow-400 underline"
            >
              Close
            </button>
          </div>
